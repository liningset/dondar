...
export default function NihilistCipher({
  ...
  let additionTypeRef = useRef(null);
  ...
  function validate(input) {
    ...
    if (seperatorIsNotEmpty) {
      if (seperatorIsValid) {
        ...
        helpers.haltMessage(opInfo, "Seperator cannot contain numbers.");
        return false;
      }
    } else {
  ...
  function encode(input) {
    ...
    return ptNums
      .map((num, i) => Number(num) + Number(keyNums[i]))
  ...
  function decode(input) {
    ...
    let ptNums = ctNums.map((ctNum, i) => {
      return Number(ctNum) - Number(keyNums[i]);
    });
    return ptNums
      .map((ptNum, i) => {
