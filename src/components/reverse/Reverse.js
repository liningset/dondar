...
export default function Reverse({ helpers, isDisabled, setOutputBinary }) {
  ...
  const normal = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!&()_[]{}?<>.,'".split(
    ""
  );
  const upsideDown = "∀ꓭϽᗡƎᖵ⅁HIᒋꓘ⅂ꟽNOԀꝹꓤSꓕՈɅϺX⅄Zɐqɔpǝⅎƃɥᴉɾʞʅɯuodbɹsʇnʌʍxʎz¡⅋)(‾][}{¿><˙',".split(
    ""
  );
  ...
  function triggerFn() {
    ...
    if (validate(inputBinary)) {
      switch (typeRef.current.value) {
        case "char":
          helpers.updateStorage({ outputBins: inputBinary.reverse() });
          break;

        case "byte":
          helpers.updateStorage({
            outputBins: inputBinary
              .join("")
              .match(/[01]{8}/g)
              .reverse()
          });
